...
一个**纯静态 HTML** 的初中数学题库项目，收录 **13 道** 初中数学题（含完整解答），以树形导航分类展示。所有解题严格基于**初中数学知识**。
...
```
/workspace/
├── build.js                 # 核心构建脚本（13 题数据 + 模板 + KaTeX 渲染 + 生成 index/题目页）
├── index.html               # 首页（左侧树形导航 + 右侧 iframe 内容区）
├── problems/                # 题目页面
│   ├── images/             # 原题图片 + 生成的 SVG（p3_ic.svg 题图 / p3_grid.svg 解答图）
│   └── p9.html … p19.html / app1.html / app2.html   # 13 个独立题目页
...
```bash
# 1) 生成全部页面（index.html + 13 个 problems/*.html）
node build.js
...
| id | 分类 | 题型 | 难度 | 标题 |
|------|--------|--------------|------|------|
| p9   | 二次根式 | 计算填空 | ★★★☆ | 第9题 最简二次根式与同类二次根式 |
| p4   | 二次根式 | 选择填空 | ★★☆☆ | 第4题 同类二次根式的判定 |
| p6   | 二次根式 | 阅读理解+计算 | ★★★☆ | 第6题 整体代入法（阅读理解） |
| p5   | 二次根式 | 计算填空 | ★★☆☆ | 第5题 整体代换与链式平方 |
| p19  | 二次根式 | 二次根式 | ★★★☆ | 化简题① 二次根式配凑 |
| app1 | 最值问题 | 应用题 | ★★★☆ | 解决问题(1) 篱笆围菜园（最值问题） |
| app2 | 最值问题 | 综合应用 | ★★★★ | 解决问题(2) 四边形面积最小值 |
| p12  | 几何综合 | 几何综合 | ★★★★ | 第12题 矩形 OABC 角度三等分（E 在延长线） |
| p14  | 几何综合 | 几何综合 | ★★★★ | 第14题 直线+正方形+平行四边形（综合） |
| p16  | 几何综合 | 几何综合 | ★★★★ | 解答题第6题 菱形中的动点（综合） |
| p15  | 函数与统计 | 选择填空 | ★★☆☆ | 选择题第2题 函数的判断 |
| p17  | 函数与统计 | 函数与统计 | ★★☆☆ | 第17题 弹簧长度与质量的函数关系 |
| p18  | 函数与统计 | 函数与统计 | ★★★☆ | 第18题 甲乙两组成绩分析（四分位数+箱线图+方差） |

**分类（树形导航 4 类）**：二次根式、最值问题、几何综合、函数与统计
