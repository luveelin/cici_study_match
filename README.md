...
**纯静态 HTML** 的初中数学题库项目，收录 **20 道父题** 初中数学题（含完整解答，附 **53 道**同类拓展子题，共 **73 个**题目页面），以树形导航分类展示。所有解题严格基于**初中数学知识**。

每道题页面包含：原题图片、✍️ 解题过程、📌 知识点总结（可折叠，默认收起）、📚 南昌/江西中考类似题（同类拓展，附真题来源链接）。

> 注：**父题页已移除「🎯 举一反三」栏目**（2026-07-12 起），仅保留「📚 同类拓展」。子题页本就不含该栏。

> 全部解答、知识点总结、中考真题均基于初中范围（二次根式、最值问题、几何综合、坐标轴几何、函数与统计、勾股数定理）。
...
├── problems-data.*.js        # 题目数据（按分类拆分为 6 个文件：二次根式/几何综合/函数与统计/勾股数定理/坐标轴几何/最值问题，从 build.js 抽出，由 build.js require 引入，共 73 题）
...
│   ├── p{N}.html（含 p{N}a{M}.html 子题页） # 独立题目页（含同类拓展子题，共 73 页；仅正文 + 外链 css/js，无内联样式/脚本）
...
```bash
# 1) 生成全部页面（index.html + 全部 problems/*.html，当前 73 页）
node build.js
...
各 `problems-data.*.js`（`build.js` 通过 `require` 引入，按分类拆分）中每个题目的数据结构：
...
```javascript
{
  ...
  parent: "p3",                           // 可选：设为父题 id → 成为其同类拓展子题，作为父题的可折叠子菜单（已废弃的 sub / problemSub/ 机制勿再用）
...
3. ✍️ 解题过程
4. 📚 南昌/江西中考类似题（同类拓展）—— 真实中考真题，附 `江西省20xx` 来源链接。**子题数量灵活：2 题或 3 题均可**（详见下方「子题机制」的同类拓展数量约定），不强制 3 题。
...
> 📌 **同类拓展子题数量约定（2026-07-15 起）**：每道父题的「📚 南昌/江西中考类似题（同类拓展）」可生成 **2 题或 3 题**子题，**不固定为 3 题**——按题目知识点覆盖面与拓展价值灵活决定，2 题够用就 2 题、需要多角度就 3 题。子题 id 仍按 `p{N}a{1}`、`p{N}a{2}`、`p{N}a{3}` 顺序编号；只生成 2 题时不留 `a3` 占位。父题页「同类拓展」栏的小题条目数与实际生成的子题数一一对应。

若某主题（如 p3）的「📚 南昌/江西中考类似题（同类拓展）」内含多道小题，可为每道小题生成**独立详细解答页**，作为父题的**子菜单**嵌在左侧导航树中；子题页内容只含解题过程与知识点总结，**不含**「🎯 举一反三」与「📚 同类拓展」栏目。

- 在对应的 `problems-data.*.js` 中将子题作为独立题目对象加入数组，并设 `parent: "<父题id>"`（如 p3a1/p3a2 设 `parent: "p3"`）。
...
| id | 题型 | 难度 | 标题（含同类拓展子题） |
|------|--------------|------|------|
| p3   | 几何综合 | ★★★☆ | 网格中的角度求和（子题 p3a1/p3a2） |
| p7   | 几何综合 | ★★★☆ | 多边形内角和的错算问题（子题 p7a1/p7a2/p7a3） |
| p8   | 几何综合 | ★★★★ | 镜面反射最短路径问题（子题 p8a1/p8a2/p8a3） |
| p10  | 几何综合 | ★★★☆ | 等腰三角形高线与角平分线（子题 p10a1/p10a2/p10a3） |
| p16  | 几何综合 | ★★★★ | 菱形中的动点（子题 p16a1/p16a2/p16a3） |

### 勾股数定理（3 父题）
| id | 题型 | 难度 | 标题（含同类拓展子题） |
|------|--------------|------|------|
| p20  | 勾股数定理 | ★★★☆ | 表格规律与勾股数（子题 p20a1/p20a2/p20a3） |
| p21  | 填空 | ★★★☆ | 完美勾股数（子题 p21a1/p21a2/p21a3） |
| p22  | 勾股数定理 | ★★★☆ | 平板支架与角平分线（子题 p22a1/p22a2/p22a3） |

### 二次根式（5 父题）
| id | 题型 | 难度 | 标题（含同类拓展子题） |
|------|--------------|------|------|
| p4   | 选择填空 | ★★☆☆ | 同类二次根式的判定（子题 p4a1/p4a2/p4a3） |
| p5   | 计算填空 | ★★☆☆ | 整体代换与链式平方（子题 p5a1/p5a2/p5a3） |
| p6   | 阅读理解+计算 | ★★★☆ | 整体代入法（子题 p6a1/p6a2/p6a3） |
| p9   | 计算填空 | ★★★☆ | 最简二次根式与同类二次根式（子题 p9a1/p9a2/p9a3） |
| p19  | 二次根式 | ★★★☆ | 二次根式配凑（子题 p19a1/p19a2/p19a3） |

### 最值问题（1 父题）
| id | 题型 | 难度 | 标题（含同类拓展子题） |
|------|--------------|------|------|
| p2   | 综合应用 | ★★★★ | 四边形面积最小值（子题 p2a1/p2a2/p2a3） |

### 函数与统计（4 父题）
| id | 题型 | 难度 | 标题（含同类拓展子题） |
|------|--------------|------|------|
| p11  | 函数与统计 | ★★★☆ | 被墨水污染数据的箱线图分析（子题 p11a1/p11a2/p11a3） |
| p15  | 选择填空 | ★★☆☆ | 函数的判断（子题 p15a1/p15a2/p15a3） |
| p17  | 函数与统计 | ★★☆☆ | 弹簧长度与质量的函数关系（子题 p17a1/p17a2/p17a3） |
| p18  | 函数与统计 | ★★★☆ | 甲乙两组成绩分析（四分位数 + 箱线图 + 方差）（子题 p18a1/p18a2/p18a3） |

> 注：早期题号 p1、p13 已不存在；所有同类拓展子题均以 `parent` 字段挂到对应父题下，作为父题的可折叠子菜单（详见「子题机制」）。

**分类（树形导航 6 类，自上而下顺序）**：坐标轴几何、几何综合、勾股数定理、二次根式、最值问题、函数与统计
...
> ⚠️ **新增题目预置标绿规则**：每次新建父题（生成对应 `problems/p{N}.html`），**可把该 id 加入 `initData.js` 的 `window.__INIT_MASTERED__` 数组**，让所有访客默认标绿。当前 `initData.js` 已预置 **20 个父题**（p4/p5/p9/p6/p7/p8/p10/p19/p15/p17/p18/p3/p16/p2/p11/p12/p14/p20/p21/p22）；**子题默认不预置标绿**，由「父菜单自动收起」逻辑统一控制（父题下子题全部掌握时才收起）。
...
- **改内容**：编辑对应分类的 `problems-data.*.js`（题目数据已从 `build.js` 抽出、按分类拆分，通过 `require` 引入）中对应题目的 `content`，然后 `node build.js` 重新生成。
...
- **新增同类拓展子题页（嵌套子菜单）**：在对应的 `problems-data.*.js` 中追加子题对象并设 `parent: "<父题id>"`（如 `parent: "p3"`），`build.js` 会将其作为父题的可折叠子菜单嵌在左侧导航树；父子题均输出到 `problems/`，图片路径用 `images/xxx.svg`。父题 `content` 用相对超链接 `<id>.html` 跳转（如 `p3a1.html`）。
  - **并同步加入预置标绿**：该新子题的 id 必须同时加入 `initData.js` 的 `window.__INIT_MASTERED__` 数组（见上「⚠️ 新增子题自动标绿规则」），否则访客看不到默认绿色 ✓。

## 开发约定

- **同类拓展子题数量灵活（2 或 3 题）**：新增父题时，「📚 南昌/江西中考类似题（同类拓展）」生成 **2 题或 3 题**子题均可，**不强制 3 题**。按知识点覆盖面决定：2 题能覆盖核心考点即可只出 2 题（如 p7 多边形内角和的错算，出"基本型 + 多算型 + 漏算型"3 题；若某题只有两个明显拓展方向，出 2 题即可）。子题 id 按 `p{N}a{1}`、`p{N}a{2}`（、`p{N}a{3}`）顺序编号，不留空位占位。
