...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* 二级叶子题：补偿父题头箭头占位(18px)，使标题与同级父题头对齐 */
  .tree-folder:not(.tree-subfolder) > .tree-children > li > .tree-file {
    padding-left: 31px;
  }
  /* 三级子题：比二级再缩进一层(18px) */
...
function toggleMastered(id) {
  ...
  // 保存到 localStorage（记录 mastered 与 unmastered 两套集合，使本地缓存优先于预置）
  let raw;
  try { raw = JSON.parse(localStorage.getItem('mastered') || '{}'); } catch (e) { raw = {}; }
  const data = Array.isArray(raw) ? { mastered: raw, unmastered: [] } : (raw || {});
  const mastered = new Set(data.mastered || []);
  const unmastered = new Set(data.unmastered || []);
  if (isMastered) { mastered.add(id); unmastered.delete(id); }
  else { mastered.delete(id); unmastered.add(id); }
  localStorage.setItem('mastered', JSON.stringify({ mastered: [...mastered], unmastered: [...unmastered] }));
  ...
  // 仅取消视觉标绿（与 markMastered 对称，供本地缓存取消预置项使用）
...
document.addEventListener('DOMContentLoaded', function() {
  const initMastered = window.__INIT_MASTERED__ || [];

  // 1) 先加载 initData.js 预置掌握（全站共享基线）
  initMastered.forEach(id => markMastered(id));

  // 2) 再检查本地缓存标记（个人，作为最终权威，可覆盖预置）
  //    mastered：用户显式打勾；unmastered：用户在预置基础上显式取消勾选
  try {
    const raw = JSON.parse(localStorage.getItem('mastered') || '{}');
    // 兼容旧版：曾用纯数组存储
    const data = Array.isArray(raw) ? { mastered: raw, unmastered: [] } : (raw || {});
    (data.mastered || []).forEach(id => markMastered(id));
    (data.unmastered || []).forEach(id => unmarkMastered(id));
  } catch (e) {}
